...
public class ComputeQFunction {
	...
	public int regress(int vfun, CAction a) {
		...
		for (String var : state_vars_in_vfun) {
			...
			_context._hsSpecialNodes.add(dd_conditional_sub);
		...
		//      ... 
		// }
		_context._hsSpecialNodes.add(a._reward);	
		...
		_camdp._context._hsSpecialNodes.add(q);
		//q = _context.reduceRound(q);
		// Optional Display
	...
	public int maxOutVar(int ixadd, String var, double lb, double ub) {
		XADDLeafMaxMin max = _context.new XADDLeafMaxMin(var, lb, ub,true, _camdp._logStream);
		ixadd  = _context.reduceProcessXADDLeaf(ixadd, max, false);
		return max._runningMaxMin;
	}
}
