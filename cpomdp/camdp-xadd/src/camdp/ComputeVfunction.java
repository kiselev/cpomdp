...
public class ComputeVfunction {
	...
	public int computeValueFunction(int vfun, CAction a) {
		...
		//for ANNOTATION: the prime values don't get equality for their annotations any more! 
		...
		for (Map.Entry<String, Integer> e : bvar_dds.entrySet()) {
			...
			//System.out.println("- Summing out: " + var_prime + "/" + var_id + " in\n" + xadd.getString(dd_mult));
...
*/		// Multiply in discount and add reward
    	q = xadd.apply(a._reward,  
				...
				XADD.SUM,-1);
    	//q = xadd.annotateXADD(q, xadd.getTermNode(ArithExpr.parse("0")) );
    	g = xadd.getGraph(q);
		g.addNode("_temp_");
