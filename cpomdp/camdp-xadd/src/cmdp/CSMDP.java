...
public class CSMDP {
	...
	public final static boolean DISPLAY_1D = true;
	...
	public ArrayList<String> _alNVars;
	...
	public HashMap<String,SAction> _hmName2Action;
	 ...
	 **/
	public CSMDP(ArrayList input) {
		_context = new XADD();
		_prevDD = _maxDD = _valueDD = null;
		_bdDiscount = new BigDecimal("" + (-1));
		_nIter = null;
		_hmName2Action = new HashMap<String,SAction>();
	...
	public int solve(int max_iter) {
		...
		while (++iter <= max_iter) {
			...
			for (Map.Entry<String,SAction> me : _hmName2Action.entrySet()) {
				...
				if (DISPLAY_1D)
				{
					for (int i=0;i<_alCVars.size();i++)
					{
					TestXADDDist.PlotXADD(_context, regr, 
							_context._hmMinVal.get(_alCVars.get(i)), 1d, _context._hmMaxVal.get(_alCVars.get(i)), _alCVars.get(i)
							, "Q-function "+iter+" action "+me.getKey());
					double integral = TestXADDDist.TestNormalize(_context, regr, _alCVars.get(i));
...
//				g2.launchViewer(1300, 770);
	...
	public void flushCaches() {
		...
		for (SAction a : _hmName2Action.values()) {
	 ...
	 **/
	public int regress(int vfun, SAction a) {
		...
		//after continuous regression, now *integrate out the noise variables *
		//the result of the previous stage has the noise variables, multiply it by the noise CPT
		// perform definite integral according to the noise ranges 
		
		for (Map.Entry<String, Integer> e : a._hmNoise2DD.entrySet()) 
		{
			String noiseParam = e.getKey();
			Integer dd_mult = e.getValue();
			System.out.println("- Summing out: " + noiseParam + "/" + 
			...
			q = _context.computeDefiniteIntegral(q, noiseParam);
			q = _context.makeCanonical(q);
		}
...
//	}
	...
	/**
	 * CSMDP construction methods, add noise
	 **/
	public void buildCMDP(ArrayList input) {
		...
		o = i.next();
		_alNVars = (ArrayList<String>) ((ArrayList) o).clone();
		...
		while (true) {
			...
			_hmName2Action.put(aname, new SAction(this, aname, cpt_map, noise_map,reward_dd));
			o=i.next(); // endaction
